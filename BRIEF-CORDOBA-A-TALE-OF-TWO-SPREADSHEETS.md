# BRIEF — CÓRDOBA: A TALE OF TWO SPREADSHEETS

### Ingram Manor LLC · 4–5 September 2026 · the seam that produces the faults also produces the findings

**Córdoba, Spain is at 37.89°N in Andalusia. Córdoba, Argentina is at 31.41°S
and has four times the people.** Both entries are correct. Both cities are real.
Nothing in any register is wrong. And three times in one session this project
put a twelfth-century Andalusian philosopher in South America.

That is the whole argument, and it runs both ways. GLOSSARY, `the water
between`: *"where the faults live. Every fault found in the long August session
lay BETWEEN two registers, each perfectly truthful about its own island."*

The same seam is also the yield. Averroes in Argentina came from the roster
meeting a gazetteer. The fall of Constantinople emptying into Florence came from
the roster meeting the events. **One session, one seam, both results.** This
brief is about what lives there.

Córdoba was caught three times by three different guards, and no two were the
same mechanism — which is the practical lesson underneath the argument, and the
reason there is more than one.

**And the title is not only a joke.** Two cities of one name is the fault. Two
spreadsheets is the fix: 500 rows of `Place`, authored by hand across two
worksheets, because the derivation that would have saved the labour put Marathon
in Provence, Waterloo in Texas and Carthage in Ohio. The second worksheet is
also what made the yield possible — 325 new points of contact for the roster to
strike. **The same pair of documents that answers the fault produces the
finding.**
