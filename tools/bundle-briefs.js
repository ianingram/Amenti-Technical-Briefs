...
/* ── WHICH REPO AM I IN? ──────────────────────────────────────────────────
   FOUND ON THE FIRST RUN, 19 AUGUST. This was uploaded to Amenti.live by
   mistake and did exactly what it was told: walked the repo it was in and
   bundled nine SURFACES — Page1, Page2, read.html, the court — as though they
   were accounts of the building. It reported the result cleanly and went
   green.

   A TOOL CANNOT KNOW WHICH REPO IT SHOULD BE IN. It can know which one it is
   in, and it can refuse when that is not the one it was written for. Same
   class of fault as a probe walking the ship when it was meant for the
   mirror: nothing broken, everything correct, and the answer about the wrong
   thing entirely. */
const here = path.basename(path.resolve(ROOT));
if (here !== 'Amenti-Technical-Briefs' && !process.argv.includes('--anywhere')) {
  console.error('REFUSES: this is `' + here + '`, not Amenti-Technical-Briefs.');
  console.error('');
  console.error('  This tool bundles the BRIEFS — shop-floor accounts of the building.');
  console.error('  Run in any other repo it will happily bundle the surfaces instead and');
  console.error('  report a clean result, which is how it spent its first run turning');
  console.error('  Page1.html into a brief.');
  console.error('');
  console.error('  Move it to Amenti-Technical-Briefs, or pass --anywhere if you mean it.');
  process.exit(2);
}
